...
export default function AdminDashboard() {
  ...
  const registeredInstructors = users
    .filter(u => u.role === 'instructor')
    .map(u => u.displayName.trim());

  const instructorsList = Array.from(new Set(['Sofía', 'Matías', 'Camila', 'Lucas', ...registeredInstructors]));
  ...
  // Instructor credentials edit state
  const [editingInstructor, setEditingInstructor] = useState<UserProfile | null>(null);
  const [instructorEditEmail, setInstructorEditEmail] = useState('');
  const [instructorEditRut, setInstructorEditRut] = useState('');
  const [instructorEditName, setInstructorEditName] = useState('');

  // New Instructor creation state
  const [showCreateInstructorModal, setShowCreateInstructorModal] = useState(false);
  const [newInstructorName, setNewInstructorName] = useState('');
  const [newInstructorEmail, setNewInstructorEmail] = useState('');
  const [newInstructorRut, setNewInstructorRut] = useState('');

  const handleOpenInstructorEdit = (instructorName: string) => {
    const existing = users.find(u => u.role === 'instructor' && u.displayName.trim().toLowerCase() === instructorName.trim().toLowerCase());
    
    const profileToEdit: UserProfile = existing || {
      userId: `inst_${instructorName.toLowerCase().replace(/í/g, 'i').replace(/á/g, 'a')}`,
      displayName: instructorName,
      email: `${instructorName.toLowerCase().replace(/í/g, 'i').replace(/á/g, 'a')}@yoga.com`,
      role: 'instructor',
      createdAt: new Date().toISOString(),
      rut: '12345678-9'
    };
    
    setEditingInstructor(profileToEdit);
    setInstructorEditEmail(profileToEdit.email);
    setInstructorEditRut(profileToEdit.rut || '');
    setInstructorEditName(profileToEdit.displayName);
  };

  const handleSaveInstructorProfile = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!editingInstructor) return;
    if (!instructorEditEmail || !instructorEditRut) {
      alert('Por favor complete todos los campos.');
      return;
    }
    
    const cleanRut = instructorEditRut.trim();
    if (!/^[0-9]+-?[0-9Kk]$/.test(cleanRut.replace(/\./g, ''))) {
      alert('Por favor ingrese un RUT válido con guión y dígito verificador (ej: 22123456-5).');
      return;
    }
    ...
    try {
      const updatedProfile: UserProfile = {
        ...editingInstructor,
        email: instructorEditEmail.trim().toLowerCase(),
        rut: cleanRut
      };
      
      await yogaDatabase.saveUserProfile(updatedProfile);
      
      const cachedUsers = await yogaDatabase.getAllUsers();
      setUsers(cachedUsers);
      setEditingInstructor(null);
      alert('Profesor/a actualizado con éxito. Ahora puede iniciar sesión con estas credenciales.');
    } catch (err: any) {
      console.error(err);
  ...
  const handleCreateInstructor = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newInstructorName.trim() || !newInstructorEmail.trim() || !newInstructorRut.trim()) {
      alert('Por favor complete todos los campos.');
      return;
    }

    const cleanRut = newInstructorRut.trim();
    if (!/^[0-9]+-?[0-9Kk]$/.test(cleanRut.replace(/\./g, ''))) {
      alert('Por favor ingrese un RUT válido con guión y dígito verificador (ej: 22123456-5).');
      return;
    }
    ...
    try {
      ...
      const nameExists = users.some(u => u.role === 'instructor' && u.displayName.trim().toLowerCase() === newInstructorName.trim().toLowerCase());
      if (nameExists) {
        alert('Ya existe un profesor registrado con ese nombre.');
        return;
      }

      // Check if email is already taken
      const emailExists = users.some(u => u.email.trim().toLowerCase() === newInstructorEmail.trim().toLowerCase());
      if (emailExists) {
        alert('Este correo electrónico ya está registrado por otro usuario.');
        return;
      }

      const cleanName = newInstructorName.trim();
      const generatedId = `inst_${cleanName.toLowerCase().replace(/í/g, 'i').replace(/á/g, 'a').replace(/[^a-z0-9]/g, '_')}_${Math.random().toString(36).substr(2, 5)}`;
      ...
      alert(`¡Profesor/a ${newProfile.displayName} creado/a con éxito!`);
    } catch (err: any) {
      console.error(err);
      alert('Ocurrió un error al registrar el nuevo profesor.');
    }
  };
  ...
  const loadAllData = async () => {
    try {
      let allPkgs = await yogaDatabase.getPackages();
      let allBookings = await yogaDatabase.getBookings();
      let allUsers = await yogaDatabase.getAllUsers();
      let allWebClasses = await yogaDatabase.getClasses();
      let allExpenses = await yogaDatabase.getExpenses();

      // Auto-populate with the comprehensive historic dataset (April to date) on first boot if DB is empty
      if (allBookings.length === 0 && allPkgs.length <= 1) {
        setIsSeeding(true);
        const result = await yogaDatabase.resetAndSeedDatabase();
        if (result.success) {
          allPkgs = await yogaDatabase.getPackages();
          allBookings = await yogaDatabase.getBookings();
          allUsers = await yogaDatabase.getAllUsers();
          allWebClasses = await yogaDatabase.getClasses();
          allExpenses = await yogaDatabase.getExpenses();
        }
        setIsSeeding(false);
      }
  ...
  const handleAssignPackage = async (e: React.FormEvent) => {
    ...
    if (studentId === 'new_student') {
      if (!newStudentName || !newStudentEmail || !newStudentRut) {
        alert('Por favor ingresa nombre, correo y RUT del nuevo alumno.');
        return;
      }
      
      const cleanRut = newStudentRut.trim().replace(/\./g, '');
      if (!/^[0-9]+-?[0-9Kk]$/.test(cleanRut)) {
        alert('Por favor ingresa un RUT válido con guión y dígito verificador (ej: 22123456-5).');
        return;
      }
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-8 relative z-10 font-sans">
      ...
      {activeTab === 'calendar' && (
        <div className="space-y-6">
          <div className="bg-white/40 backdrop-blur-lg border border-white/50 p-6 rounded-2xl shadow-sm space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 pt-4 border-t border-white/20">
              {instructorsList.map((instructor) => {
      ...
      {activeTab === 'instructors' && (
        <div className="space-y-6">
          <div className="bg-white/40 backdrop-blur-lg border border-white/50 p-6 rounded-2xl shadow-sm space-y-6">
            ...
            {instructorsSubTab === 'teachers' ? (
              <div className="space-y-4">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-[#e0f0e3]/40 p-4 rounded-2xl border border-emerald-300/30 gap-3">
                  <div>
                    <h4 className="text-xs font-black uppercase text-emerald-850 tracking-wider">Gestión de Profesores</h4>
                    <p className="text-[10px] text-slate-500 font-bold mt-0.5">
                      Profesores registrados: {instructorsList.length} • Pueden ingresar con su correo y últimos 4 números de su RUT como clave.
                    </p>
                  </div>
                  <button
                    onClick={() => setShowCreateInstructorModal(true)}
                    className="px-4 py-2 bg-emerald-700 hover:bg-emerald-800 text-white font-black text-xs flex items-center gap-2 rounded-xl transition-all shadow-sm active:scale-95 cursor-pointer"
                  >
                    <span>➕ Registrar Profesor/a</span>
                  </button>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
                  {instructorStudentsAndBookings.map((instObj) => {
                  const matchInstProfile = users.find(u => u.role === 'instructor' && u.displayName.trim().toLowerCase() === instObj.instructorName.trim().toLowerCase());
                  const instEmail = matchInstProfile?.email || `${instObj.instructorName.toLowerCase().replace(/í/g, 'i').replace(/á/g, 'a')}@yoga.com`;
                  const instRut = matchInstProfile?.rut || 'Sin RUT';
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-white/20 pb-3.5">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="flex flex-wrap gap-1.5 items-center mt-1">
                              <span className="text-[10px] font-bold text-emerald-850 bg-emerald-50 px-1.5 py-0.5 rounded border border-emerald-100 leading-tight">
                              ...
                              <span className="text-[10.5px] text-slate-650 font-bold font-mono bg-slate-100 px-1.5 py-0.2 rounded border border-slate-200">
                                RUT: {instRut}
                              </span>
                            </div>
                            <p className="text-[10.5px] text-slate-500 font-bold mt-1 inline-block">{instEmail}</p>
                          </div>
                        </div>

                        <div className="flex flex-row sm:flex-col items-center sm:items-end justify-between w-full sm:w-auto gap-2">
                          <button
                            onClick={() => handleOpenInstructorEdit(instObj.instructorName)}
                            className="px-2.5 py-1 text-slate-705 bg-white hover:bg-slate-100 border border-slate-205 rounded-lg text-[9px] font-black transition-all active:scale-95 shadow-sm"
                          >
                            ⚙️ Cambiar RUT/Email
                          </button>
                          <span className="text-xs font-black text-slate-900 block bg-emerald-105 text-emerald-955 px-2 py-0.5 rounded-md border border-emerald-100 shadow-sm leading-tight text-center">
                  ...
                  >
                    ...
                    <div className="flex justify-between items-center border-b border-white/20 pb-3.5">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="font-black text-slate-900 text-base">{studentName}</h3>
                          <div className="flex items-center flex-wrap gap-2 mt-0.5">
                            <span className="text-[11px] text-slate-600 font-bold">{studentEmail}</span>
                            {user?.rut && (
                              <span className="text-[10px] font-mono font-black text-emerald-900 bg-emerald-50 px-1.5 py-0.2 rounded border border-emerald-150">
                                RUT: {user.rut}
                              </span>
                            )}
                          </div>
                ...
                <form onSubmit={handleAssignPackage} className="space-y-4 text-xs">
                  ...
                  {selectedStudentId === 'new_student' && (
                    ...
                    >
                      ...
                      <div className="space-y-1">
                        <label className="font-bold text-slate-750 block text-[11px]">Correo Electrónico Alumno</label>
                        <input
                          type="email"
                          required
                      ...
                      <div className="space-y-1">
                        <label className="font-bold text-slate-750 block text-[11px]">RUT del Alumno (Formato: 22123456-5)</label>
                        <input
                          type="text"
                          required
      ...
      {/* EDIT INSTRUCTOR CREDENTIALS MODAL */}
      <AnimatePresence>
        {editingInstructor && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
            <motion.div 
            ...
            >
              <div className="bg-emerald-800 text-white p-5 flex justify-between items-center">
                <div>
                  <h4 className="font-extrabold text-sm uppercase tracking-wider">
                    Credenciales de {editingInstructor.displayName}
                  </h4>
                  <p className="text-[10px] text-emerald-100 font-bold mt-0.5 uppercase tracking-wide">
                    Establecer Correo y RUT para Inicio de Sesión
                  </p>
                </div>
                <button 
              ...
              <form onSubmit={handleSaveInstructorProfile} className="p-6 space-y-4">
                <div className="space-y-1">
                  ...
                  <input
                    type="text"
                    disabled
                    value={instructorEditName}
                    className="w-full bg-slate-100 border border-slate-205 rounded-lg p-2.5 text-xs font-bold text-slate-500 focus:outline-none"
                  />
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <input
                    type="email"
                    required
                    placeholder="ej. sofia@yoga.com"
                    value={instructorEditEmail}
                    onChange={(e) => setInstructorEditEmail(e.target.value)}
                    className="w-full bg-slate-50 border border-slate-200 rounded-lg p-2.5 text-xs font-bold text-slate-800 focus:outline-none focus:ring-1 focus:ring-emerald-500"
                  />
                </div>
                ...
                <div className="space-y-1">
                  <label className="block text-[10px] font-black uppercase tracking-wider text-slate-500">
                    RUT del Instructor (Formato: 12345678-9)
                  </label>
                  <input
                    type="text"
                    required
                    placeholder="ej. 15263748-9"
                    value={instructorEditRut}
                    onChange={(e) => setInstructorEditRut(e.target.value)}
                    className="w-full bg-slate-50 border border-slate-200 rounded-lg p-2.5 text-xs font-bold text-slate-800 focus:outline-none focus:ring-1 focus:ring-emerald-500"
                  />
                  <p className="text-[10px] text-slate-500 font-medium">
                    Su contraseña de ingreso serán los 4 números del RUT elegidos antes del guión.
                  </p>
                </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
            ...
            >
              <div className="bg-emerald-800 text-white p-5 flex justify-between items-center">
                <div>
                  <h4 className="font-extrabold text-sm uppercase tracking-wider">
                    Registrar Nuevo/a Profesor/a
                  </h4>
                  <p className="text-[10px] text-emerald-100 font-bold mt-0.5 uppercase tracking-wide">
                    Establecer Nombre, Correo y RUT
                  </p>
                </div>
                <button 
              ...
              <form onSubmit={handleCreateInstructor} className="p-6 space-y-4">
                ...
                <div className="space-y-1">
                  <label className="block text-[10px] font-black uppercase tracking-wider text-slate-500">
                    RUT del Profesor (Formato: 12345678-9)
                  </label>
                  <input
                    type="text"
                    required
                    placeholder="ej. 12345678-9"
                    value={newInstructorRut}
                    onChange={(e) => setNewInstructorRut(e.target.value)}
                    className="w-full bg-slate-50 border border-slate-200 rounded-lg p-2.5 text-xs font-bold text-slate-800 focus:outline-none focus:ring-1 focus:ring-emerald-500"
                  />
                  <p className="text-[10px] text-slate-500 font-medium font-bold">
                    Su contraseña de ingreso serán los últimos 4 dígitos del RUT antes del guión.
                  </p>
                </div>
