...
export default function LoginScreen({ onLoginSuccess }: LoginScreenProps) {
  ...
  const handleCredentialsLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 4. Extract expected password from RUT
      let expectedPassword = '1234'; // Default fallback
      if (matchedUser.rut) {
        expectedPassword = getPasswordFromRut(matchedUser.rut);
      }

      if (loginPassword.trim() !== expectedPassword) {
        setError(
          'Contraseña incorrecta. Recuerda que tu contraseña corresponde a los últimos 4 dígitos de tu RUT (antes del guión). Ejemplo: si tu RUT es 12345678-9, tu contraseña es 5678.'
        );
        setLoading(false);
        return;
      }

      // 5. Log in simulated/active user session
      await yogaAuth.simulateUserLogin(matchedUser.email, matchedUser.role, matchedUser.displayName);
      onLoginSuccess();
    } catch (err: any) {
      console.error(err);
