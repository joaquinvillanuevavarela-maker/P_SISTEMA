...
// Dynamic helper to check if sandbox (mock) mode is active or simulated profile is logged in
export const isUsingMock = (): boolean => {
  return isMock || localStorage.getItem('simulated_active_user') !== null;
};
  ...
  if (!localStorage.getItem(LS_KEYS.USERS)) {
    const defaultUsers: Record<string, UserProfile> = {
      ...
      'student_demo': {
        ...
        displayName: 'Joaquín Villanueva',
        role: 'student',
        createdAt: new Date().toISOString(),
        rut: '21286220-7'
      },
    ...
    // Correct extant local user list for Joaquin and Valentina
    const existingUsersRaw = localStorage.getItem(LS_KEYS.USERS);
    if (existingUsersRaw) {
      try {
        ...
        // Correct Joaquin
        if (users['student_demo']) {
          if (users['student_demo'].rut !== '21286220-7' || users['student_demo'].displayName !== 'Joaquín Villanueva' || users['student_demo'].role !== 'student' || users['student_demo'].email !== 'joaquinvillanuevavarela@gmail.com') {
            users['student_demo'].displayName = 'Joaquín Villanueva';
            users['student_demo'].email = 'joaquinvillanuevavarela@gmail.com';
            users['student_demo'].rut = '21286220-7';
            users['student_demo'].role = 'student';
            updated = true;
          }
        }
        ...
        // Correct Valentina
        if (users['admin_val']) {
          if (users['admin_val'].rut !== '22222222-2' || users['admin_val'].email !== 'valentina@gmail.com' || users['admin_val'].role !== 'admin') {
            users['admin_val'].email = 'valentina@gmail.com';
            users['admin_val'].rut = '22222222-2';
            users['admin_val'].role = 'admin';
            updated = true;
          }
        }
        ...
        console.error("Failed to correct users in localStorage", e);
      }
    }
  }

  // Also correct loaded session if Joaquín or Valentina is active
  const activeUserRaw = localStorage.getItem(LS_KEYS.ACTIVE_USER) || localStorage.getItem('simulated_active_user');
  if (activeUserRaw) {
    try {
      const active = JSON.parse(activeUserRaw);
      if (active) {
        let updated = false;
        if (active.email === 'joaquinvillanuevavarela@gmail.com') {
          if (active.rut !== '21286220-7' || active.displayName !== 'Joaquín Villanueva' || active.role !== 'student') {
            active.displayName = 'Joaquín Villanueva';
            active.rut = '21286220-7';
            active.role = 'student';
            updated = true;
          }
        } else if (active.email === 'valentina@yoga.com' || active.email === 'valentina@gmail.com') {
          if (active.email !== 'valentina@gmail.com' || active.rut !== '22222222-2' || active.role !== 'admin') {
            active.email = 'valentina@gmail.com';
            active.rut = '22222222-2';
            active.role = 'admin';
            updated = true;
          }
        }
        if (updated) {
          localStorage.setItem(LS_KEYS.ACTIVE_USER, JSON.stringify(active));
          localStorage.setItem('simulated_active_user', JSON.stringify(active));
        }
      }
    } catch (e) {
      console.error(e);
    ...
    const defaultPackages: ClassPackage[] = [
      {
        ...
        studentName: 'Joaquín Villanueva',
      ...
      unsubscribeFirebase = auth.onAuthStateChanged(async (firebaseUser: FirebaseUser | null) => {
        ...
        try {
          ...
          if (profileDoc.exists()) {
            ...
            if (profile.email === 'joaquinvillanuevavarela@gmail.com') {
              if (profile.rut !== '21286220-7' || profile.displayName !== 'Joaquín Villanueva' || profile.role !== 'student') {
                profile.rut = '21286220-7';
                profile.displayName = 'Joaquín Villanueva';
                profile.role = 'student';
                try {
                  setDoc(doc(db, 'users', firebaseUser.uid), profile);
                } catch (e) {
                  console.error("Failed to update Joaquin profile in Firestore:", e);
                }
              }
            }
            ...
            const isAdminEmail = firebaseUser.email === "valentina@gmail.com";
  ...
  simulateUserLogin: async (email: string, role: UserRole, displayName: string) => {
    ...
    if (!isMock && auth && db) {
      try {
        ...
        // 1. Check if there's an existing profile with this email in the database (created by admin)
        ...
        // 2. Draft the new profile preserving historic parameters like RUT
        user = {
          userId: uid,
          ...
          rut: existingProfile?.rut || ''
        };
        
        // 3. Save new profile
        await setDoc(doc(db, 'users', uid), user);
        
        // 4. Clean up the old profile document if it was a placeholder (different from uid)
        if (oldUserId) {
          try {
            await deleteDoc(doc(db, 'users', oldUserId));
          } catch (e) {
            console.warn("Could not delete old placeholder profile:", e);
          }
          
          // 5. Migrate any packages that were linked to the old placeholder ID to the new authentic UID
          try {
            const pkgsSnap = await getDocs(query(collection(db, 'packages'), where('studentId', '==', oldUserId)));
            for (const pDoc of pkgsSnap.docs) {
              await updateDoc(doc(db, 'packages', pDoc.id), { studentId: uid });
            }
          } catch (e) {
            console.error("Failed to migrate packages to new user ID:", e);
          }
          
          // 6. Migrate any bookings that were linked to the old placeholder ID to the new authentic UID
          try {
            const bookingsSnap = await getDocs(query(collection(db, 'bookings'), where('studentId', '==', oldUserId)));
            for (const bDoc of bookingsSnap.docs) {
              await updateDoc(doc(db, 'bookings', bDoc.id), { studentId: uid });
            }
          } catch (e) {
            console.error("Failed to migrate bookings to new user ID:", e);
          }
        }
      } catch (err) {
        console.info("Anonymous authentication in Live Mode not active, falling back:", err);
...
export const yogaDatabase = {
  ...
  getAllUsers: async (): Promise<UserProfile[]> => {
    if (isUsingMock()) {
      ...
      // Assure Joaquín in simulated list has correct credentials
      if (users['student_demo']) {
        users['student_demo'].displayName = 'Joaquín Villanueva';
        users['student_demo'].rut = '21286220-7';
        users['student_demo'].role = 'student';
      ...
      // Assure Valentina in simulated list has correct credentials
      if (users['admin_val']) {
        users['admin_val'].displayName = 'Valentina (Propietaria)';
        users['admin_val'].rut = '22222222-2';
        users['admin_val'].role = 'admin';
        users['admin_val'].email = 'valentina@gmail.com';
      }
      ...
      const path = 'users';
      try {
        // Automatically authenticate anonymously to avoid unauthenticated read bans in security rules
        if (auth && !auth.currentUser) {
          try {
            await signInAnonymously(auth);
          } catch (e) {
            console.info("Anonymous authentication in getAllUsers skipped (typically disabled in project):", e);
          }
        }
        ...
        // Ensure Joaquin's user profile is 100% up-to-date in Firestore list if retrieved, or write it back.
        let joaquin = list.find(u => u.email === 'joaquinvillanuevavarela@gmail.com');
        if (joaquin) {
          if (joaquin.rut !== '21286220-7' || joaquin.displayName !== 'Joaquín Villanueva' || joaquin.role !== 'student') {
            joaquin.rut = '21286220-7';
            joaquin.displayName = 'Joaquín Villanueva';
            joaquin.role = 'student';
            try {
              setDoc(doc(db, 'users', joaquin.userId), joaquin);
            } catch (e) {
              console.error("Async restore of Joaquin in DB failed:", e);
            }
          }
        } else {
          // Add Joaquín to list and persist
          const generatedId = 'student_demo';
          const joaquinProfile: UserProfile = {
            userId: generatedId,
            email: 'joaquinvillanuevavarela@gmail.com',
            displayName: 'Joaquín Villanueva',
            role: 'student',
          ...
          list.push(joaquinProfile);
          try {
            setDoc(doc(db, 'users', generatedId), joaquinProfile);
          } catch(e) {
            console.error("Async save of manual joaquin in DB failed:", e);
          }
        }

        // Ensure Valentina's user profile is 100% up-to-date in Firestore list if retrieved, or write it back.
        let adminVal = list.find(u => u.email === 'valentina@gmail.com');
        if (adminVal) {
          if (adminVal.rut !== '22222222-2' || adminVal.displayName !== 'Valentina (Propietaria)' || adminVal.role !== 'admin') {
            adminVal.rut = '22222222-2';
            adminVal.displayName = 'Valentina (Propietaria)';
            adminVal.role = 'admin';
            try {
              setDoc(doc(db, 'users', adminVal.userId), adminVal);
            } catch (e) {
              console.error("Async restore of Valentina in DB failed:", e);
            }
          }
        } else {
          // Add Valentina to list and persist
          const valProfile: UserProfile = {
            userId: 'admin_val',
            email: 'valentina@gmail.com',
            displayName: 'Valentina (Propietaria)',
            role: 'admin',
          ...
          list.push(valProfile);
          try {
            setDoc(doc(db, 'users', 'admin_val'), valProfile);
          } catch(e) {
            console.error("Async save of manual Valentina in DB failed:", e);
          }
        }
  ...
  getPackages: async (): Promise<ClassPackage[]> => {
    if (isUsingMock()) {
      ...
      try {
        let queryRef;
        if (activeUserProfile && activeUserProfile.role === 'student' && auth.currentUser) {
          queryRef = query(collection(db, path), where('studentId', '==', auth.currentUser.uid));
        } else {
  ...
  resetAndSeedDatabase: async (): Promise<{ success: boolean; count: number; error?: string }> => {
    try {
      ...
      const mockBookingsToSeed: Booking[] = [
        ...
        { bookingId: 'b_apr_4', classId: 'sat_1', className: 'Ashtanga Pro Sábado', instructor: 'Camila', studentId: 'student_demo', studentName: 'Joaquín Villanueva', studentEmail: 'joaquinvillanuevavarela@gmail.com', classDate: '2026-04-11', classTime: '08:30', status: 'attended', createdAt: '2026-04-10T15:00:00.000Z' },
        ...
        { bookingId: 'b_apr_11', classId: 'mon_1', className: 'Vinyasa Flow', instructor: 'Sofía', studentId: 'student_demo', studentName: 'Joaquín Villanueva', studentEmail: 'joaquinvillanuevavarela@gmail.com', classDate: '2026-04-27', classTime: '07:00', status: 'attended', createdAt: '2026-04-26T14:00:00.000Z' },
        ...
        { bookingId: 'b_may_14', classId: 'fri_1', className: 'Vinyasa Energizante', instructor: 'Sofía', studentId: 'student_demo', studentName: 'Joaquín Villanueva', studentEmail: 'joaquinvillanuevavarela@gmail.com', classDate: '2026-05-22', classTime: '07:00', status: 'attended', createdAt: '2026-05-21T09:00:00.000Z' },
        ...
        { bookingId: 'b_jun_8', classId: 'mon_1', className: 'Vinyasa Flow', instructor: 'Sofía', studentId: 'student_demo', studentName: 'Joaquín Villanueva', studentEmail: 'joaquinvillanuevavarela@gmail.com', classDate: '2026-06-08', classTime: '07:00', status: 'attended', createdAt: '2026-06-07T19:00:00.000Z' },
        ...
        { bookingId: 'b_jun_13', classId: 'sat_1', className: 'Ashtanga Pro Sábado', instructor: 'Camila', studentId: 'student_demo', studentName: 'Joaquín Villanueva', studentEmail: 'joaquinvillanuevavarela@gmail.com', classDate: '2026-06-13', classTime: '08:30', status: 'attended', createdAt: '2026-06-12T16:00:00.000Z' },
